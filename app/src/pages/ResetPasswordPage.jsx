...
import { API_URL, API_PASSWORD } from '../config/api.config';
...
const ResetPasswordPage = () => {
	...
	useEffect(() => {
		const fetchToken = async () => {
			const tokenContent = await axios.get(API_URL + API_PASSWORD + token)
	...
	const submitPassword = async (form) => {
		...
		try {
			const response = await axios.post(API_URL + API_PASSWORD + token, form)
			if (response.status === 204) {
				navigate("/login", { state: { login: userToken.user.email, password: form.password } });
			}
		} catch (error) {
			throw error
		}
	...
	return (
		<div className="container max-w-xl p-5">
			<div className='border rounded bg-white p-5'>
				...
				<Form onSubmit={handleSubmit(onSubmit)} isDisabled={isSubmitting} isLoading={isSubmitting} className="flex flex-col gap-5 items-center">
					<FormInput
						type="password"
						name="password"
