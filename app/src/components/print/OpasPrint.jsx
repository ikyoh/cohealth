...
const MyDoc = ({ datas, mission, patient }) => {
    ...
    const calcTotalServiceTime = (service) => {
        ...
        if (service.periodicity === "jour")
            return (
                ...
                calcNumberOfDays(datas.content.beginAt, datas.content.endAt)
            );
        if (service.periodicity === "semaine")
            return (
                Number(service.time) *
                Number(service.frequency) *
                calcNumberOfWeeks(datas.content.beginAt, datas.content.endAt)
            );
        if (service.periodicity === "mois")
            return (
                Number(service.time) *
                Number(service.frequency) *
                calcNumberOfMonths(datas.content.beginAt, datas.content.endAt)
            );
    };
    ...
    if (mission.assurance.npa) assurance += mission.assurance.npa + ", ";
    if (mission.assurance.city) assurance += mission.assurance.city + ", ";
    assurance += "\n";
    ...
    doctor += "\n";
    if (mission.doctor.address1) doctor += mission.doctor.address1 + ", ";
    if (mission.doctor.npa) doctor += mission.doctor.npa + ", ";
    if (mission.doctor.city) doctor += mission.doctor.city + ", ";

    doctor += "\n";
    if (mission.doctor.phone) doctor += "Tél. : " + mission.doctor.phone;
            ...
            <Page size="A4" style={styles.page} dpi={dpi} wrap debug={false}>
                ...
                <View style={styles.row}>
                    <View style={styles.column}>
                        ...
                        <Separator />
                        <Text style={styles.text}>{patientInfos}</Text>
                    </View>
                    <View style={styles.column}>
                        ...
                        <View style={{ flexDirection: "row" }}>
                            ...
                            >
                                ...
                                >
                                    ...
                                    {"\n"}
                                    Période :{" "}
                                    {dayjs(datas.content.beginAt).format(
                                        "L"
                                    )}{" "}
                                    au{" "}
                                    {dayjs(datas.content.endAt).format("L") +
                                        " "}
                                    (
                                    {calcNumberOfDays(
                                        datas.content.beginAt,
                                        datas.content.endAt
                                    )}{" "}
                                    jours)
                                </Text>
                ...
                >
                    <View style={{ marginHorizontal: gap / 2 }}>
                        <Text style={styles.text}>
                            Total A (Évaluation et conseils) :{" "}
                            {datas.content.totalA} min.
                            {datas.content.totalA !== 0 &&
                                " (" +
                                    Math.round(
                                        (datas.content.totalA * 100) / 60
                                    ) /
                                        100 +
                                    "h)"}
                            {"\n"}
                            Total B (Examens et traitements) :{" "}
                            {datas.content.totalB} min.
                            {datas.content.totalB !== 0 &&
                                " (" +
                                    Math.round(
                                        (datas.content.totalB * 100) / 60
                                    ) /
                                        100 +
                                    "h)"}
                            {"\n"}
                            Total C (Soins de base) : {
                                datas.content.totalC
                            }{" "}
                            min.
                            {datas.content.totalC !== 0 &&
                                " (" +
                                    Math.round(
                                        (datas.content.totalC * 100) / 60
                                    ) /
                                        100 +
                                    "h)"}
                        </Text>
                    ...
                    <View style={{ marginHorizontal: gap / 2 }}>
                        <Text style={styles.textBold}>
                            Total :{" "}
                            {datas.content.totalA +
                                datas.content.totalB +
                                datas.content.totalC}{" "}
                            min. (
                            {Math.round(
                                ((datas.content.totalA +
                        ...
                        {mission.user.signature && (
                            <>
                                <Image
                                    ...
                                    source={
                                        URL + mission.user.signature.contentUrl
                                    }
                                />
                            </>
                        )}
            ...
            <Page size="A4" style={styles.page} dpi={dpi} wrap debug={false}>
                    ...
                    <View style={styles.column}>
                        ...
                        >
                            ...
                            <Text style={{ fontSize: "9px" }}>
                                Description détaillée de la prestation (selon
                                article 7, al. 2 OPAS)
                            </Text>
                        </View>
                        <Separator />
                ...
                <View style={styles.row}>
                    <View style={styles.column}>
                        ...
                        <Text style={{ fontSize: "8px", lineHeight: 1.4 }}>
                            Code * : Selon le catalogue des actes de l'ASSASD
                            (novembre 2015)
                        </Text>
                    </View>
                </View>
