...
const fetchDoctorComment = ({ queryKey }) => {
    ...
    return request({ url: API + "?exists[doctor]=true&doctor=" + doctorID, method: 'get' })
}

const fetchAssuranceComment = ({ queryKey }) => {
    const assuranceID = queryKey[2]
    return request({ url: API + "?exists[assurance]=true&assurance=" + assuranceID, method: 'get' })
}
...
staleTime: 60000,
        enabled: id ? true : false,
        select: data => {return data['hydra:member'][0]}
    })
}
