...
import { API_ASSURANCES as API, itemsPerPage } from '../config/api.config'
import _ from 'lodash'


/* CONFIG */
const queryKey = 'assurances'
...
const fetchPaginatedDatas = (page, sortValue, sortDirection, searchValue) => {
    return request({ url: API + "?page=" + page + "&itemsPerPage=" + itemsPerPage + "&order[" + sortValue + "]=" + sortDirection + "&search[id,company,organization,type,phone,email,gln]=" + searchValue, method: 'get' })
}
