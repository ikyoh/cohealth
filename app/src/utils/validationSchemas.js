...
const emailRequired = (error) => Yup.string().email(error ? error : 'Email incorrect').required("Champ obligatoire")
const bool = () => Yup.boolean().required("Doit être accepter.").oneOf([true], "Doit être accepter.")
...
export const patientIRI = Yup.object({
    patient: Yup.string().required('Choix obligatoire').typeError('Choix obligatoire')
})

export const doctorIRI = Yup.object({
    doctor: Yup.string().required('Choix obligatoire').typeError('Choix obligatoire')
})
