...
import { cantons, legalCares, services_family } from '../utils/arrays';
    ...
    return (
        <>
            <FormSelect
                ...
                label="Famille"
                error={name && errors[name] ? errors[name]['family'] : errors['family']}
                register={register}
                required={true}
            >
            ...
            <FormSelect
                ...
                error={name && errors[name] ? errors[name]['opas'] : errors['opas']}
                register={register}
                required={true}
            >
                <option value="">Choisir un article</option>
                {legalCares.map((l) =>
                    <option key={uuid()} value={l}>{l}</option>
                )}
            </FormSelect>
            <FormTextarea
                name={name ? name + ".title" : "title"}
                label="Intitulé"
                error={name && errors[name] ? errors[name]['title'] : errors['title']}
                register={register}
                required={true}
                rows={2}
            />
            <FormInput
                ...
                label="Numéro d'acte"
                placeholder=""
                error={name && errors[name] ? errors[name]['act'] : errors['act']}
                register={register}
                required={true}
            />
            <FormInput
                ...
                error={name && errors[name] ? errors[name]['time'] : errors['time']}
                register={register}
                required={true}
            />
            <FormTextarea
